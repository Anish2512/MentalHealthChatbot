...
Remember to maintain a non-judgmental and compassionate tone while providing the answer
    ...
    prompt1 = PromptTemplate(input_variables=[ "input"], template=friend)
    prompt2 = PromptTemplate(input_variables=[ "input"], template=mentalassistant)
...
def personalise(answer,name):
    ...
    prompt="""Given the following answer from a ChatGPT API call and the user's name I want you to \
    personalise this response with the user's name if possible. It should look natural, and the answer \
    must be comforting the user.\n\nUSER NAME: \n{name}\n\nAnswer: {answer}\n\nPersonalised response:"""
    prompt3 = PromptTemplate(input_variables=["name", "answer"], template=prompt)
    llm2 = ChatOpenAI(model_name="gpt-3.5-turbo", openai_api_key="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",temperature=0)



    # prompt_template = prompt1
    # prompt = PromptTemplate(template=prompt_template, input_variables=["input"])
    chain2 = LLMChain(llm=llm2, prompt=prompt3)
    hello1=chain2.run(name=name,answer=answer) #chain to get response from GPT for users question, response stored in hello
    print("before json loads")
    print(hello1)
    #answer2=json.loads(hello1)
    return hello1
...
---------------------
u are a mental health chatbot
---------------------
Given the context information and not prior knowledge, answer the query in a dict format with the keys  in curly bracket format.the keys in the output must by Question,Answer,Followup and Suicidal(True or False ).The keys and answer must be enclosed in double inverted commas.just give the output.no extra text or formatting.The answer must start with curly braces and end with the same.no extra commas,double commas or mention of json while giving the answer as this will lead to bad readabilty.
Query: {user_input}
Answer:
"""
    messages = [
    ...
    hello = chat_response.choices[0].message.content
    #,chat_history=memory#chain to get response from GPT for users question, response stored in h
    print("before json loads")
    print(hello)
...
def maps_post():
    ...
    request_data = request.get_json()
    pincode= request_data["Pincode"]
    new_data=gethospitals.get_nearest_hospitals(pincode,'AIzaSyBIAHitGC-B1gn-CJuJeW86FMb2SgaI-WI')
    Assistant={
